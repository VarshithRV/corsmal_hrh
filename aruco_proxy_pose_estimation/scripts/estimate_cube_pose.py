            ...
            for i, marker_id in enumerate(ids.flatten()):
                if marker_id == 21:
                    ...
                    if success:
                        ...
                        # Apply transformation t to calculate cube pose
                        t = np.array([0.03, 0.03, 0.03])  # Example transformation
                        cube_position = tvec.flatten() + t

                        cube_pose = PoseStamped()
                        cube_pose.header.stamp = rospy.Time.now()
                        cube_pose.header.frame_id = "camera_color_optical_frame"  # Set the appropriate frame ID
                        cube_pose.pose.position.x = cube_position[0]
                        cube_pose.pose.position.y = cube_position[1]
                        cube_pose.pose.position.z = cube_position[2]
